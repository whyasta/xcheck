...
func (l Logger) Trace(ctx context.Context, begin time.Time, fc func() (string, int64), err error) {
	...
	case err != nil && l.LogLevel >= gormlogger.Error && (!l.IgnoreRecordNotFoundError || !errors.Is(err, gorm.ErrRecordNotFound)):
		sql, rows := fc()
		logger.Error("trace query", zap.String("sql", sql), zap.Error(err), zap.Duration("elapsed", elapsed), zap.Int64("rows", rows))
	case l.SlowThreshold != 0 && elapsed > l.SlowThreshold && l.LogLevel >= gormlogger.Warn:
		sql, rows := fc()
		logger.Warn("trace query", zap.String("sql", sql), zap.Duration("elapsed", elapsed), zap.Int64("rows", rows))
	case l.LogLevel >= gormlogger.Info:
		sql, rows := fc()
		logger.Debug("trace query", zap.String("sql", sql), zap.Duration("elapsed", elapsed), zap.Int64("rows", rows))
	}
}
